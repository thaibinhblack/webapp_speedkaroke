...
<div id="page-cameara">
    <f7-block>
        ...
        <f7-col width="33" class="media-cameara" v-for="(image,index) in images" :key="index"><img :src="image.URL_IMAGE" height="100%" height="100%"></f7-col>
           ...
           const self = this;
            ...
            for (let index = 0; index < files.length; index++) {
               storageRef.child('cameara/'+files[index].name).put(files[index]).then(function(snapshot) {
                  ...
                  storageRef.child('cameara/'+files[index].name).getDownloadURL().then((url) => {

                      self.axios.post(self.$store.state.API_URL + 'image/upload?firebase=1&UUID_BAR_KARAOKE='+self.$f7route.params.UUID+'&URL_IMAGE='+url).then((response) => {
