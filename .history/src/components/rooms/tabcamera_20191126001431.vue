...
<div id="page-cameara">
    <f7-block>
        ...
        <f7-col width="33" class="media-cameara" v-for="(image,index) in images" :key="index"><img :src="image.URL_IMAGE" height="100%" width="100%"  @click="$refs.pageCamera.open()"></f7-col>
...
export default {
    ...
    methods:{
            ...
            for (let index = 0; index < files.length; index++) {
               storageRef.child(files[index].name).put(files[index]).then(function(snapshot) {
                  ...
                  storage.child(files[index].name).getDownloadURL().then((url) => {
                    ...
                    console.log('test',self.photos)
                    data.append("UUID_ROOM_BAR_KARAOKE", self.$f7route.params.UUID)
        ...
        api_image()
        {
            ...
            this.axios.get(this.$store.state.API_URL + 'image/'+this.$f7route.params.UUID+'?type=UUID_ROOM_BAR_KARAOKE').then((response) => {
