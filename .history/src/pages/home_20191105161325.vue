...
export default {
  components: {
    'tab-your-karaoke': () => import('@/components/karaokes/tabyourkaraoke.vue')
  },
  data()
  {
    return {
  ...
  methods: {
    getKaraokeNew()
    {
      this.axios.get(this.$store.state.API_URL + "karaoke?page=1").then((response) => {
    ...
    {
      thix.axios.post(this.$store.state.API_URL + "province/search",search).then((response) => {
                    this.karaoke_new = response.data
                    this.karaoke_tmp = response.data
