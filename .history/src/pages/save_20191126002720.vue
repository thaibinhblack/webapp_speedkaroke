...
<f7-page>
   ...
   <f7-tabs>
       ...
       <f7-tab id="tab-bill">
           <f7-block>
               <f7-card v-for="(bill,index) in bills" :key="index" style="padding:10px;">
                   <f7-row>
                       <f7-col width="30">
...
export default {
    ...
    methods:{
        ...
        api_like_room()
        {
            console.log('api_rooom')
            this.axios.get(this.$store.state.API_URL + 'like_karaoke?UUID_ROOM=all&api_token='+this.$session.get('token')).then((response) => {
