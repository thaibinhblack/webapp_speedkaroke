...
export default {
    ...
    methods:{
        ...
        uploadImage()
        {
            const files = this.$refs.uploadImage.files
            const data = new FormData()
            data.append("IMAGES",files)
            data.append("UUID_BAR_KARAOKE",this.$f7.params.UUID)
            this.axios.post(this.$store.state.API_URL + 'image/upload/mobile',data).then((response) => {
