      ...
      <f7-row>
        <f7-col v-for="(karaoke,index) in karaoke_new" :key="index" width="50" style="padding:4px">
          <f7-card class="item-karaoke" style="margin:0">
              <img clas="image-karaoke" width="100%" height="100px" :src="'http://127.0.0.1:8000'+karaoke.BANNER_BAR_KARAOKE" :alt="karaoke.NAME_BAR_KARAOKE">
...
export default {
  ...
  methods: {
    getKaraokeNew()
    {
      axios.get("http://127.0.0.1:8000/api/v1/karaoke?page=1").then((response) => {
      ...
      navigator.geolocation.getCurrentPosition(function(location) {
       axios.get('https://api.opencagedata.com/geocode/v1/json?q='+location.coords.latitude+'+'+location.coords.longitude+'&key=2d3fed69d9404855bf934ad52e5c1d65&pretty=1')
                .then((response) => {
                  ...
                  {
                    this.GPS = response.data.results[0].components.city
                    search.append("NAME_RPOVINCE", response.data.results[0].components.city)
                  }
                  else{
