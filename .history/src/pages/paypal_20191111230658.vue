...
export default {
    ...
    watch:
    {
        code_promotio(newVal)
        {
            this.api_check_promotino(newVal)
        }
    },
    methods: {
        api_booking()
        ...
        api_check_promotino(code)
        {
            this.axios.get('http://127.0.0.1:800/api/v1/promotion/'+this.booking.UUID_BAR_KARAOKE+'/check?CODE_PROMOTION='+code).then((response) =>{
                console.log(response.data)
            })
        }
    },
