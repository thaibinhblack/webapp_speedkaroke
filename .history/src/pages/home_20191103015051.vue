<template>
  <f7-page name="home">
    ...
    <f7-navbar class="tool-bar">
        ...
        <f7-subnavbar :inner="false">
          <f7-row class="top-header">
              <f7-col width="50" class="search-left">
               <f7-link href="/search/" style="color:#333;padding:0"> <f7-icon  ios="material:search" aurora="material:search" md="material:search"></f7-icon>
                <span class="text-search">Tìm địa điểm karaoke</span></f7-link>
              </f7-col>
    ...
    <f7-toolbar bottom tabbar labels>
      ...
      <f7-link href="/login/" tab-link="#login">
...
export default {
  ...
  methods: {
    ...
    {
      this.axios.post( this.$store.state.API_URL + "province/search",search).then((response) => {
                    this.karaoke_new = response.data
                    this.karaoke_tmp = response.data
...
.top-header {margin: 7px;border:1px solid #e2e2e2;border-radius: 5px;background: #fff;}
...
.text-search {font-size: 12px;}
.search-left {line-height: 36px}
</style>