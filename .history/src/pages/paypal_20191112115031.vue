...
<f7-page>
    ...
    <f7-block>
        ...
        <f7-list>
            <f7--list-item title="Hình thức thanh toán" smart-select :smart-select-params="{openIn: 'sheet'}">
...
export default {
    ...
    methods: {
        ...
        success_bill()
        {
            ...
            let self = this;
            this.axios.post(this.$store.state.API_URL+'bill?api_token='+this.$session.get('token'),bill).then((response) => {
                self.$f7.dialog.alert(response.data.message)
