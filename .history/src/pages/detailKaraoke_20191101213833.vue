...
<f7-page name="detail-karaoke">
    ...
    <f7-block>
        ...
        <f7-icon v-for="i in Math.floor(karaoke.STAR_RATING)" :key="i" color="yellow" ios="material:star" aurora="material:star" md="material:star"></f7-icon> ({{karaoke.NUMBER_REATED}})
        ...
        ApiGetImages()
        {
            ...
            this.axios.get(this.$store.state.API_URL + 'image/'+this.$f7route.params.UUID+'?type=UUID_BAR_KARAOKE').then((response) => {
                response.data.forEach((image) => {
                    this.photos.push({
                        url: 'http://127.0.0.1:8000' + image.URL_IMAGE
                    })
                })
            })
        }
