    ...
    <f7-navbar class="tool-bar">
        ...
        <f7-subnavbar :inner="false">
          <f7-row class="top-header">
              <f7-col width="10">
                <f7-icon material="search" size="24" style="line-height:30px"></f7-icon>  
              ...
              <f7-col width="40" class="title-gps">
                ...
                <f7-list class="list-province">
                  <f7-list-item smart-select :smart-select-params="{openIn: 'popup', searchbar: true, searchbarPlaceholder: 'Tìm tỉnh / thành phố'}">
                    <select name="province" v-model="province_selected">
                      <option v-for="(province,index) in provinces" :key="index" :value="province.ID_PROVINCE" :selected="province.ID_PROVINCE == province_selected">{{province.NAME_PROVINCE}}</option>
                    </select>
                  </f7-list-item>
                </f7-list>
              </f7-col>
