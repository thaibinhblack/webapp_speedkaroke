...
<f7-page dark>
    <f7-list class="list-accout info">
        <f7-list-item :title="user.EMAIL" after="xem thông tin" link="/info/" badge-color="#fff">
            <img v-if="user.TYPE_USER == 'FACEBOOK'" width="48px" height="48px" slot="media"  :src="user.AVATAR" alt="">
            <img v-else-if="user.TYPE_USER == 'GOOGLE'" width="48px" height="48px" slot="media"  :src="user.AVATAR" alt="">
            <img v-else :src="$store.state.PUBLIC_URL + user.AVATAR" width="48px" height="48px" slot="media">
        </f7-list-item>
    </f7-list>
    <f7-list class="list-accout">
          <f7-list-item title="Lịch sử đặt phòng" link="/save#tab-history/">
    ...
    <p align="center" color="red" @click="logout()" style="cursor: pointer;color:red;">Đăng xuất</p>
