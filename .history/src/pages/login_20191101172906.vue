    ...
    methods: {
        ...
        var provider =  new firebase.auth.FacebookAuthProvider();
            ...
            const router = self.$f7router;
              firebase.auth().signInWithPopup(provider).then((result) => {
                ...
                data.append("EMAIL",result.credential.user.email)
                app.dialog.preloader()
                self.axios.post(self.$store.state.API_URL + 'facebook',data).then(() =>{
                    self.$session.start()
                    self.$session.set('token',result.credential.accessToken)
                    router.navigate('/')
                })
      ...
      signInGoogle()
      {
         var provider_google =  new firebase.auth.GoogleAuthProvider();
            ...
            user =  result.user;
             console.log('google 1',result)
            // ...
            }).catch(function(error) {
              console.log('google 1',error)
            // Handle Errors here.
