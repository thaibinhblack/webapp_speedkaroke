...
<f7-page name="detail-karaoke">
    ...
    <f7-block>
        ...
        <f7-card v-for="(room,index) in rooms" :key="index">
            <img :src="$store.state.PUBLIC_URL + room.IMAGE_ROOM_BAR_KARAOKE" :alt="room.NAME_ROOM_BAR_KARAOKE">
            <div class="content-room">
                ...
                <p style="margin:0"><v-icon size="12" color="yellow" v-for="i in room.STAR_RATING" :key="i">mdi-star</v-icon> <small>({{room.NUMBER_RATED}} đánh giá), {{room.VIEW_ROOM}} lượt xem</small></p>
            </div>
        </f7-card>
    </f7-block>
    </f7-page>
...
export default {
    ...
    methods:{
        ...
        ApiGetRoom(UUID)
        {
            this.axios.get(this.$store.state.API_URL + "room?UUID_BAR_KARAOKE="+ UUID).then((response) => {
