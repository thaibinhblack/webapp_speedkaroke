...
<f7-page name="detail-karaoke">
    ...
    <f7-toolbar top tabbar labels>
      ...
      <f7-link tab-link="#tab-camera" class="camera">
    ...
    <f7-tabs animated>
        <f7-tab id="tab-karaoke" class="page-content page-tab">
                ...
                <f7-row>
                    <f7-col width="100">
                        <f7-card v-for="(room,index) in rooms" :key="index">
                           <f7-link :href="'/room/'+room.UUID_ROOM_BAR_KARAOKE+'/'">
        ...
        <tab-camera @uploadImage="url = $event"/>
        <f7-tab id="tab-rating" class="page-content page-tab">
        <f7-block>
            <f7-block-title>Đánh giá karaoke</f7-block-title>
            <f7-icon v-for="(star ,index) in stars" class="star-rating" :key="index" :class="star.class" @click="rating(star.value)"  ios="material:star" aurora="material:star" md="material:star">
            </f7-icon>
        </f7-block>
        </f7-tab>
...
export default {
    components: {
        'tab-camera': () => import('@/components/tabCamera.vue')
    },
    data()
    {
        return {
    ...
    watch: {
        ...
        url(newVal)
        {
            ...
            data.append("URL_IMAGE",newVal)
            this.axios.post(this.$store.state.API_URL + 'image?firebase=1',data).then((response) => {
...
.star-rating.active {color: yellow}
.star-rating:hover {color: yellow}
.page-tab {padding: 0}
</style>