        ...
        <f7-tab id="room" tab-active>
            <div class="tab-room">
                ...
                <f7-block>
                    ...
                    {{user_booking}}
                    <f7-button v-if="user_booking == -1" @click="book()" class="btn-book">Đặt phòng ngay</f7-button>
...
export default {
    ...
    methods:
    {
        ...
        cancle()
        {
            const self = this;
            const app = self.$f7;
            this.axios.get(this.$store.state.API_URL + 'cancle?api_token='+this.$session.get('token')+'&UUID_ROOM_BAR_KARAOKE='+this.room.UUID_BARR_KARAOKE).then(() => {
