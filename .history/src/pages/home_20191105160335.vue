<template>
  <f7-page name="home">
    ...
    <f7-navbar class="tool-bar">
        ...
        <f7-subnavbar :inner="false">
          <f7-row class="top-header">
              <f7-col width="65" class="search">
                 
                <f7-link href="/search" style="text-align:left;justify-content: left;color:#333;">  
                  <f7-icon ios="material:search" aurora="material:search" md="material:search"></f7-icon> 
                  <small style="margin-left:5px;">Nhập từ khóa tìm kiếm karaoke</small>
                </f7-link>
              </f7-col>
              <f7-col width="35" class="title-gps">
                
                <f7-list class="list-province">
                  <f7-list-item title="Tỉnh / TP" smart-select :smart-select-params="{openIn: 'popup', searchbar: true, searchbarPlaceholder: 'Tìm tỉnh / thành phố'}">
    ...
    <f7-block>
      <f7-row>
        <f7-col class="text-new tab active">
          <f7-link tab-link="#karaoke-new" tab-link-active>
             <span class="tabs" >Mới nhất</span>
          </f7-link>
        </f7-col>
        <f7-col class="text-gps">
          <f7-link tab-link="#your-karaoke">
              <span class="tabs">Gần</span>
          </f7-link>
        </f7-col>
      </f7-row>
    </f7-block>
    <f7-tabs>
        <f7-tab id="karaoke-new" tab-active>
            ...
            <f7-row>
              <f7-col v-for="(karaoke,index) in karaoke_new" :key="index" width="50" style="padding:4px">
                <f7-card class="item-karaoke" style="margin:0">
                    <f7-link :href="'/karaoke/'+karaoke.UUID_BAR_KARAOKE+'/'" ><img clas="image-karaoke" width="100%" height="160px" :src="$store.state.PUBLIC_URL+karaoke.BANNER_BAR_KARAOKE" :alt="karaoke.NAME_BAR_KARAOKE"></f7-link>
    ...
    <f7-link tab-link="#tab-save"  href="/save/" no-fast-click	no-link-class>
      <f7-icon ios="material:today" aurora="material:note" md="material:note">
      </f7-icon>
      <span class="tabbar-label">Lưu các địa điểm</span>
    </f7-link>
    <f7-link href="/login/" no-fast-click	no-link-class	tab-link="#login">
...
export default {
  ...
  methods: {
    getKaraokeNew()
    {
      axios.get(this.$store.state.API_URL + "karaoke?page=1").then((response) => {
    ...
    getAddress(location)
    {
      axios.get('https://api.opencagedata.com/geocode/v1/json?q='+location.coords.latitude+'+'+location.coords.longitude+'&key=2d3fed69d9404855bf934ad52e5c1d65&pretty=1')
                .then((response) => {
    ...
    getKaraokeGPS(search)
    {
      axios.post("http://127.0.0.1:8000/api/v1/province/search",search).then((response) => {
                    this.karaoke_new = response.data
                    this.karaoke_tmp = response.data
...
.search {height: 44px;line-height: 42px;}
</style>