<template>
<f7-page>
    <f7-navbar  back-link="Back" :back-link-url="'/karaoke/'+UUID_BARR_KARAOKE+'/'">{{room.NAME_ROOM_BAR_KARAOKE}}</f7-navbar>
    ...
    methods:
    {
        ApiGetRoom()
        {
            this.axios.get(this.$store.state.API_URL + 'room/'+this.$f7route.params.UUID).then((response) => {
                ...
                this.UUID_BARR_KARAOKE = response.data.UUID_BARR_KARAOKE
